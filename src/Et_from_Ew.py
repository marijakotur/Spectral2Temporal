...
class SpectrometerCamera(QtWidgets.QWidget):
    ...
    def calculateBox(self):
        ...
        temp_centroid = np.sum(self.Intt_mask*self.t)/np.sum(self.Intt_mask)
        print 'temp centroid', temp_centroid
        tukey_lim_r = temp_centroid - self.goalTempWidth/2
        tukey_lim_l = temp_centroid + self.goalTempWidth/2
        ...
        self.box *= np.sum(self.Intt_mask)/np.sum(self.box)
        self.plotTemporalWidget.plot15.setData(self.t,self.box)   
        #self.temp_box_diff = np.sum((self.box*self.Intt_mask)**2)
        self.temp_box_diff = np.sum(np.abs(self.box-self.Intt_mask))/np.sum(self.box)
        print self.temp_box_diff
        self.ampLossStr = str(self.ampLoss)
        self.ampMaskLossLabel.setText(self.ampLossStr)
        self.boxDiffLabel.setText(str(self.temp_box_diff))
    ...
    def updateFFT(self):
        ...
        self.geomPhaseCoef = self.geomPhaseCoefSpinbox.value()
...
#         self.spectralPhaseSlider.slider.setValue(self.geomPhaseCoef)
#         self.spectralPhaseSlider.label.setText(str(self.geomPhaseCoef))
...
#         self.controlsLayout.addWidget(self.spectralPhaseSlider)
        ...
        self.geomPhaseCoefSpinbox = QtWidgets.QDoubleSpinBox()
        self.geomPhaseCoefSpinbox.setDecimals(2)
        self.geomPhaseCoefSpinbox.setValue(self.geomPhaseCoef)
        self.geomPhaseCoefSpinbox.valueChanged.connect(self.updateEverything)
        self.geomPhaseCoefSpinbox.setMinimum(-150)
        self.geomPhaseCoefSpinbox.setMaximum(150)
        self.geomPhaseCoefLabel = QtWidgets.QLabel()
        self.geomPhaseCoefLabel.setText('geom phase coef')
        ...
        self.controlsLayout.addWidget(self.geomPhaseCoefLabel)
        self.controlsLayout.addWidget(self.geomPhaseCoefSpinbox)
        ...
        self.boxRadioButton = QtWidgets.QCheckBox()
        self.boxRadioButton.toggled.connect(self.calculateBox)
        self.boxRadioButtonLabel = QtWidgets.QLabel('box?')
        self.boxDiffLabel = QtWidgets.QLabel()
        self.boxDiffLabel.setText(str(self.boxDiff))
        
        self.controlsLayout.addWidget(self.boxRadioButtonLabel)
        self.controlsLayout.addWidget(self.boxRadioButton)
        self.controlsLayout.addWidget(self.boxDiffLabel)
        self.controlsLayout.addItem(vspaceritem)
